...
public:
    ...
    struct Material {
        ...
        [[nodiscard]] bool hasTexture() const { return !imageData.empty() && imageW > 0 && imageH > 0; }
    };
    ...
    [[nodiscard]] bool isEmpty() const { return vertices.empty() || triangles.empty(); }
    ...
    [[nodiscard]] std::array<Vertex, 3> getTriangleVertices(const Triangle &tri) const;

    // Sample the color at a given triangle + barycentric weights (bary.x for
    ...
    [[nodiscard]] glm::vec3 sampleColor(const Triangle &tri,
