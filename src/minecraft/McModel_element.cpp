...
namespace {
    ...
    AabbKey makeKey(const glm::vec3 &f, const glm::vec3 &t) {
        AabbKey k{};
        std::memcpy(&k.fx, &f.x, 4);
        std::memcpy(&k.fy, &f.y, 4);
        std::memcpy(&k.fz, &f.z, 4);
        std::memcpy(&k.tx, &t.x, 4);
        std::memcpy(&k.ty, &t.y, 4);
        std::memcpy(&k.tz, &t.z, 4);
        return k;
    }
} // anonymous namespace
...
static void writeCompactStr(std::string &out, const nlohmann::ordered_json &j, int indent = 0) {
    if (j.is_object()) {
        ...
        bool allPrim = true;
        for (const auto &e: j) if (e.is_object() || e.is_array()) {
            allPrim = false;
            break;
        }
        if (allPrim) {
            ...
            for (const auto &e: j) {
                ...
                first = false;
                out += tab;
                writeCompactStr(out, e, indent + 1);
            }
            out += '\n';
            out.append(indent, '\t');
            out += ']';
