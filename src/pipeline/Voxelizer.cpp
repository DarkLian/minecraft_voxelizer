...
// ─────────────────────────────────────────────────────────────────────────────
// voxelize — surface voxelization with per-face-direction triangle selection.
//
...
VoxelGrid Voxelizer::voxelize(const Mesh &mesh) const {
    ...
    // Per-voxel overall: for the solid flag / fallback color
        ...
        for (int z = zMin; z <= zMax; z++) {
            for (int y = yMin; y <= yMax; y++) {
                for (int x = xMin; x <= xMax; x++) {
                    ...
                    // ── Overall voxel solid + fallback color ──────────────────────
                    // Always sample the real color here — used as fallback when
                    ...
                    if (!grid.isSolid(x, y, z)) {
                        grid.set(x, y, z, color); // real color, not gray placeholder
                        filledCount++;
...
bool Voxelizer::triangleOverlapsVoxel(
    const glm::vec3 &v0, const glm::vec3 &v1, const glm::vec3 &v2,
    const glm::vec3 &voxelCenter, const glm::vec3 &halfSize) {
    ...
    float mn, mx, boxR;
    for (auto edge: edges) {
        for (auto boxAxe: boxAxes) {
            glm::vec3 axis = glm::cross(edge, boxAxe);
...
int Voxelizer::floodFillSolid(VoxelGrid &grid) const {
    ...
    for (int z = 0; z < rz; z++)
        for (int y = 0; y < ry; y++)
            for (int x = 0; x < rx; x++)
                if ((x == 0 || x == rx - 1 || y == 0 || y == ry - 1 || z == 0 || z == rz - 1) && !grid.
                    isSolid(x, y, z)) {
